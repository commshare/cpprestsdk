...
namespace SimpleWeb {
  ...
  protected:
    ...
    public:
      ...
      Connection(std::shared_ptr<ScopeRunner> handler_runner_, long timeout, Args &&... args) noexcept
          : handler_runner(std::move(handler_runner_)), timeout(timeout), /*创建了一个socket*/socket(new socket_type(std::forward<Args>(args)...)) {}
  ...
  protected:
    ...
    std::unordered_set<std::shared_ptr<Connection>> connections; //连接指针的集合
    ...
    std::shared_ptr<Connection> get_connection() noexcept {
      ...
      if(!connection) {
        ...
        connections.emplace(connection); //set
      }
